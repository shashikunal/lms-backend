  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!title || !price || !category) {
        return next(new ErrorHandler("Title, price, and category are required", 400));
      }
      ...
      const existingSku = await ProductModel.findOne({ sku: productSku });
      if (existingSku) {
        return next(new ErrorHandler("Product SKU already exists", 400));
      }
...
export const getFeaturedProducts = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const products = await ProductModel.find({ isFeatured: true, isPublished: true })
        .populate("category", "name slug")
        .limit(8);
